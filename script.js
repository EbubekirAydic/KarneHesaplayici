...
function addLesson(name = "", term1 = [0, 0, 0, 0], term2 = [0, 0, 0, 0], hours = 0) {
    ...
    lessonContainer.innerHTML = `
        ...
        <input type="number" class="score term2" placeholder="Not 1" value="${term2[0]}">
...
function calculateReportCard() {
    ...
    let totalScoreTerm1 = 0;

    let totalScoreTerm2 = 0;
    ...
    lessons.forEach(lesson => {
        ...
        totalScoreTerm1 += avgTerm1 * weeklyHours;

        totalScoreTerm2 += avgTerm2 * weeklyHours;

        totalHours += weeklyHours;

    });

    const finalTerm1Score = totalScoreTerm1 / totalHours || 0;

    const finalTerm2Score = totalScoreTerm2 / totalHours || 0;

    const finalScore = (finalTerm1Score + finalTerm2Score) / 2 || 0;

    document.getElementById("term1Result").innerText = `1. Dönem Ortalaması: ${finalTerm1Score.toFixed(2)}`;

    document.getElementById("term2Result").innerText = `2. Dönem Ortalaması: ${finalTerm2Score.toFixed(2)}`;

    document.getElementById("finalResult").innerText = `Genel Ortalama: ${finalScore.toFixed(2)}`;
